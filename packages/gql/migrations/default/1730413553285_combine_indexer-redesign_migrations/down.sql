...
DROP FUNCTION public.get_formatted_tokens_with_performance_intervals_within_period(p_start timestamp with time zone, p_end timestamp with time zone, p_interval interval, p_intervals text DEFAULT '1 minute'::text);
...
CREATE OR REPLACE VIEW "public"."formatted_tokens_with_performance" AS 
 ...
 SELECT i.token_id,
   ...
   FROM (interval_stats i
     LEFT JOIN after_intervals ai ON ((i.token_id = ai.token_id)))
  GROUP BY i.token_id, i.mint, i.decimals, i.name, i.symbol, i.platform, i.latest_price, i.initial_price, i.trades, i.latest_created_at, i.interval_start;
