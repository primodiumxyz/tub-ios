...
describe("GetTokenLiveData (old) benchmarks", () => {
  let tokens: string[] = [];
  ...
  it("should measure direct Hasura performance", async () => {
    const metric = await benchmark<"GetTokenLiveDataQuery_old">({
      identifier: "Direct Hasura hit",
      exec: async (i) => {
        const client = await createClientNoCache();
        return await client.db.GetTokenLiveDataQuery_old({
          token: tokens[i],
        });
      },
  ...
  it("should measure warm cache performance", async () => {
    ...
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      await client.db.GetTokenLiveDataQuery_old({
        token: tokens[i],
      });
    }
  ...
  afterAll(() => {
    logMetrics(metrics);
    writeMetricsToFile(metrics, "GetTokenLiveData_old");
  });
});
