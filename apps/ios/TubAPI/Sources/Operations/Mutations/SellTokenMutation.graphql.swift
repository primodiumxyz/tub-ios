...
public class SellTokenMutation: GraphQLMutation {
  ...
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"mutation SellToken($wallet: String!, $token: String!, $amount: numeric!, $override_token_price: numeric) { sell_token( args: { user_wallet: $wallet token: $token amount_to_sell: $amount token_cost: $override_token_price } ) { __typename id } }"#
    ))
