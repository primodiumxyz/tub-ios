...
public class SubAccountTokenBalanceCreditSubscription: GraphQLSubscription {
  public static let operationName: String = "SubAccountTokenBalanceCredit"
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"subscription SubAccountTokenBalanceCredit($accountId: uuid!, $tokenId: uuid!) { token_transaction_aggregate( where: { account_transaction_data: { account: { _eq: $accountId } } token: { _eq: $tokenId } transaction_type: { _eq: "credit" } } ) { __typename aggregate { __typename sum { __typename amount } } } }"#
    ))
