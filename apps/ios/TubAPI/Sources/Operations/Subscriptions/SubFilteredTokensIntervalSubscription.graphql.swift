...
public class SubFilteredTokensIntervalSubscription: GraphQLSubscription {
  ...
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"subscription SubFilteredTokensInterval($interval: interval = "30s") { formatted_tokens_interval( args: { interval: $interval } where: { is_pump_token: { _eq: true } } order_by: { trades: desc } ) { __typename token_id mint name symbol description uri supply decimals mint_burnt freeze_burnt is_pump_token increase_pct trades volume latest_price created_at } }"#
    ))

  public var interval: GraphQLNullable<Interval>

  public init(interval: GraphQLNullable<Interval> = "30s") {
