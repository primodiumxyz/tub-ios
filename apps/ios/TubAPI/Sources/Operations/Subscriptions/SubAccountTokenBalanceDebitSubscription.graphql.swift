...
public class SubAccountTokenBalanceDebitSubscription: GraphQLSubscription {
  public static let operationName: String = "SubAccountTokenBalanceDebit"
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"subscription SubAccountTokenBalanceDebit($accountId: uuid!, $tokenId: uuid!) { token_transaction_aggregate( where: { account_transaction_data: { account: { _eq: $accountId } } token: { _eq: $tokenId } transaction_type: { _eq: "debit" } } ) { __typename aggregate { __typename sum { __typename amount } } } }"#
    ))
