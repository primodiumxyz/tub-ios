...
public class SubTokenCandlesSubscription: GraphQLSubscription {
  ...
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"subscription SubTokenCandles($token: String!, $since: timestamptz = "now()", $candle_interval: interval = "1m") { token_trade_history_candles( args: { candle_interval: $candle_interval } where: { token_mint: { _eq: $token }, bucket: { _gte: $since } } ) { __typename bucket open_price_usd close_price_usd high_price_usd low_price_usd } }"#
    ))
