...
public class GetTokenCandlesSinceQuery: GraphQLQuery {
  ...
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"query GetTokenCandlesSince($token: String!, $since: timestamptz = "now()") { token_candles_history_1min(args: { token_mint: $token, start: $since }) { __typename bucket open_price_usd close_price_usd high_price_usd low_price_usd volume_usd has_trades } }"#
    ))
