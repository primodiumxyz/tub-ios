...
public class GetBulkTokenLiveDataQuery: GraphQLQuery {
  ...
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"query GetBulkTokenLiveData($tokens: [String!]!) { token_stats_interval_comp( where: { token_mint: { _in: $tokens } } args: { interval: "30m", recent_interval: "2m" } ) { __typename token_mint latest_price_usd total_volume_usd total_trades price_change_pct recent_volume_usd recent_trades recent_price_change_pct token_metadata_supply token_metadata_decimals } }"#
    ))
