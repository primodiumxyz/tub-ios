...
public class GetFilteredTokensQuery: GraphQLQuery {
  ...
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"query GetFilteredTokens($since: timestamptz!, $minTrades: bigint!, $minIncreasePct: float8!) { GetFormattedTokens( where: { created_at: { _gte: $since } trades: { _gte: $minTrades } increase_pct: { _gte: $minIncreasePct } } ) { __typename token_id mint name symbol latest_price increase_pct trades created_at } }"#
    ))
