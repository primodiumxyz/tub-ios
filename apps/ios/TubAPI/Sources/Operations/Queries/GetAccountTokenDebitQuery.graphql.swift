...
public class GetAccountTokenDebitQuery: GraphQLQuery {
  public static let operationName: String = "GetAccountTokenDebit"
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"query GetAccountTokenDebit($accountId: uuid!, $tokenId: uuid!) { token_transaction_aggregate( where: { account_transaction_relationship: { account: { _eq: $accountId } } token: { _eq: $tokenId } transaction_type: { _eq: "debit" } } ) { __typename aggregate { __typename sum { __typename amount } } } }"#
    ))
