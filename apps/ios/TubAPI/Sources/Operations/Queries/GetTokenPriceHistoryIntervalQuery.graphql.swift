...
public class GetTokenPriceHistoryIntervalQuery: GraphQLQuery {
  public static let operationName: String = "GetTokenPriceHistoryInterval"
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"query GetTokenPriceHistoryInterval($token: uuid, $start: timestamptz = "now()", $interval: interval!) { token_price_history_offset( args: { offset: $interval } where: { created_at_offset: { _gte: $start }, token: { _eq: $token } } order_by: { created_at: desc } ) { __typename created_at price } }"#
    ))
