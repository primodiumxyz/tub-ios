...
public class GetTokenMetadataQuery: GraphQLQuery {
  ...
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"query GetTokenMetadata($token: String!) { token_metadata_formatted(where: { mint: { _eq: $token } }) { __typename mint name symbol image_uri supply decimals description external_url is_pump_token } }"#
    ))
  ...
  public struct Data: TubAPI.SelectionSet {
    ...
    public static var __selections: [ApolloAPI.Selection] { [
      .field("token_metadata_formatted", [Token_metadata_formatted].self, arguments: ["where": ["mint": ["_eq": .variable("token")]]]),
    ] }
