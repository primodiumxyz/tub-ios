...
export const getRandomCoin = (index?: number): CoinData => {
  const randomIndex = index ?? Math.floor(Math.random() * COINS.length);
  const supply = Math.floor(Math.random() * 1000000000).toString();
  return { ...COINS[randomIndex], supply };
};
...
export const lamportsToSol = (lamports: bigint) => {
  ...
  if (Math.abs(raw) < 0.001) return raw;
